...
describe('Product e2e test', () => {
  ...
  describe('Create flow', () => {
    it('should load create Product page', async () => {
      ...
      expect(await updatePage.title.getAttribute('id')).to.match(/storeApp.product.home.createOrEditLabel/);
    });
    ...
    it('should create and save Products', async () => {
      ...
      await selectLastOption(updatePage.psizeSelect);

      await waitUntilDisplayed(updatePage.imageInput);
      await updatePage.imageInput.sendKeys(absolutePath);

      // await selectLastOption(updatePage.productCategorySelect);
    ...
    describe('Details, Update, Delete flow', () => {
      after(async () => {
        ...
        expect(await deleteDialog.title.getAttribute('id')).to.match(/storeApp.product.delete.question/);
