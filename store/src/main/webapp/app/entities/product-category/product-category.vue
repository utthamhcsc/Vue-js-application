<template>
  <div>
    <h2 id="page-heading" data-cy="ProductCategoryHeading">
      ...
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          ...
          <span v-text="$t('storeApp.productCategory.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ProductCategoryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('storeApp.productCategory.home.createLabel')"> Create a new Product Category </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && productCategories && productCategories.length === 0">
      <span v-text="$t('storeApp.productCategory.home.notFound')">No productCategories found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="productCategories">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('storeApp.productCategory.name')">Name</span></th>
            <th scope="row"><span v-text="$t('storeApp.productCategory.description')">Description</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="storeApp.productCategory.delete.question" data-cy="productCategoryDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-productCategory-heading" v-text="$t('storeApp.productCategory.delete.question', { id: removeId })">
